...
public final class FlyA extends PositionCheck {
  ...
  public void process(PositionUpdate positionUpdate) {
    ...
    boolean illegal =
        playerData.getPositionManager().getTouchingClimbable().get()
            || playerData.getPositionManager().getTouchingLiquid().get();
    ...
    // Added check for air ticks, because this falses on some positions when you are in air only for
    // 1-2 ticks
