...
public enum ExemptType {
  ...
  TELEPORTING(
      playerData ->
          playerData.getActionManager().getTeleported().get()
              || playerData.getTicks().get() - playerData.getJoined().get() < 100L),
  ...
  CHUNK(
      playerData ->
          !playerData
              ...
              .getWorld()
              .isChunkLoaded(
                  playerData.getPositionUpdate().get().getTo().getBlockX() << 4,
  ...
  VELOCITY(
      playerData ->
          playerData.getVelocityManager().getMaxVertical() > 0.0
              || playerData.getVelocityManager().getMaxHorizontal() > 0.0),

  /**
   * Returns true if the player was in/near/on a vehicle. Not as prone to spoofing as they actually
   * have to be near a vehicle.
   */
  VEHICLE(
      playerData ->
