...
public final class AlertManager {
  ...
  private final String broadcast =
      ColorUtil.format(
          "&8[&7Degrade&8] &a%s &7was found using an unfair advantage and was removed from the"
              + " network.");
  ...
  public void fail() {
    ...
    int violations =
        (int)
            (alerts.stream()
                .filter(violation -> violation + 9000L > System.currentTimeMillis())
                .count());
    ...
    if (violations > threshold) {
      // Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "ban " + player.getName() + " [Frequency]
      // Unfair Advantage");;
